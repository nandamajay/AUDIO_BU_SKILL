...
def build_prompt(task_spec: dict[str, Any]) -> str:
    ...
    return (
        ...
        "(evidence file path, IPCAT doc id, or kernel file path). Each entry in "
        "`nearest_targets` MUST include a non-empty `citations` list — at least "
        "one kernel DTS/patch path, candidate_patch_series id, or evidence file "
        "that directly supports the ranking. An uncited ranked target is not "
        "auditable and will be rejected. NEVER finalize the power model: set "
        "power_model.needs_review = true always. If evidence is insufficient to "
        "identify something, say so in missing_evidence and lower the confidence "
        ...
        "IPCAT was unhelpful.\n\n"
        "Also populate `element_counts`: for each enumerable audio element class "
        "you can identify (soundwire_master, dmic_line, amplifier, speaker, "
        "dai_link, audioreach_port, dsp_subsystem_instance, lpass_macro_instance), "
        "report the NUMBER OF PHYSICAL INSTANCES as typed integers per source "
        "lane — `dt` (instances actually present in the applied kernel DT at the "
        "pinned HEAD), `evidence` (from schematic/offline evidence), `proposal` "
        "(what your proposed case wires). Report the true instance count, NOT the "
        "number of distinct part numbers: two WSA8845 amplifiers driving two "
        "speakers is `amplifier` proposal 2 even though it is one part number, and "
        "eight DMICs is `dmic_line` evidence 8 even if you list them as one 'DMIC' "
        "row. Set a lane to null if you did not consult it; set 0 only when you "
        "affirmatively determined there are none. If the audio scaffolding is not "
        "yet applied to the kernel tree, set `dt` 0 AND `dt_applied` false. If a "
        "count cannot be resolved to a single integer, set `ambiguous` true and "
        "explain in `ambiguity_note` rather than guessing. Leave `catalog` null "
        "(it comes from a source not available this session). Cite every count. "
        "Omit an element class entirely if the target has no such element.\n\n"
