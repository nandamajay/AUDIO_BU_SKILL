...
def _clean_nord_facts() -> TrustedFacts:
    """Synthetic Nord facts: all four I2S pins MATCH; firmware + compatible MATCH.
    ...
    rows = [
        ...
        _row("T1", "gpio.i2s.bclk", "MATCH"),
        _row("T1", "gpio.i2s.lrclk", "MATCH"),
        _row("T1", "gpio.i2s.dout", "MATCH"),
...
def test_missing_i2s_pin_fixme_marker() -> None:
    ...
    up the gap.
    """
    # Same as _clean_nord_facts() but WITHOUT ``T1.gpio.i2s.lrclk``.
    rows = [
        _row("T1", "gpio.i2s.mclk", "MATCH"),
        _row("T1", "gpio.i2s.bclk", "MATCH"),
        # lrclk deliberately absent
        _row("T1", "gpio.i2s.dout", "MATCH"),
        _row("T5", "dts.firmware", "MATCH"),
        _row("T5", "dts.compatible", "MATCH"),
    ]
    ...
    assert "FIXME(lrclk)" in text, (
        f"missing FIXME(lrclk) marker in generated bytes:\n{text}"
    )
    # Present pins do NOT get FIXME markers.
    for present in ("mclk", "bclk", "dout"):
    ...
    assert partial.subject == "gpio.i2s.lrclk"
